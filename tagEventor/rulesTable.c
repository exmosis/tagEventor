...
rulesTableAddEntry(void)
{
    ...
    /*
     * TODO figure out where to save this type of config stuff???? via
     * GConf or something
     */
    ...
    /*
     * now make the global variable for the entry array point to the new
     * one
     */
    ...
    /*
     * paste in some text for now, although it's not yet editable by the
     * user
     */
...
rulesTableSave(void)
{
    ...
    /*
     * TODO where to save this type of config stuff???? via GConf or
     * something
     */
...
rulesTableRead(void)
{
    ...
    /*
     * TODO need to figure out how many tags we have before allocating
     * memory!
     */
    numTagEntries = NUM_DEFAULT_COMMANDS;

    /*
     * TODO we should also check for duplicate tag entries, or we could
     * allow that. Need to change code that executes scripts to allow
     * that though ...
     */
    /*
     * TODO read the current config from the widgets into a temporary
     * table, checking syntax for each and maybe checking execute
     * permissions, etc
     */

    /* allocate memory for the array of tag entries found */
    size = (numTagEntries * sizeof(tRulesTableEntry));
    tagEntryArray = malloc(size);   /* TODO check about alignment and
                     * need to pad out */

    /*
     * TODO figure out where to read/save this type of config stuff????
     * via GConf or something
     */
    for (i = 0; i < numTagEntries; i++) {
        /*
         * malloc each string for new entry added and add to
         * tagEntryArray
         */
...
static int execScript(const char *scriptPath, const char *argv0, const char *tagUID, const char *eventTypeString ){
     ...
     * occurred. The return value is -1, and the global variable errno
...
)
{
    ...
    /*
     * fork a copy of myself for executing the script in the child
     * process using exec()
     */
    pid = fork();
    if (pid < 0) {      /* PARENT process - fork error */
    ...
    if (pid > 0) {      /* PARENT process - fork worked */
        ...
        readersLogMessage(&readerManager, LOG_INFO, 2, messageString);
        return (TRUE);  /* we will have to assume the exec in the
                 * child will work */
    }
...
unsigned char rulesTableEventDispatch(tEventType eventType, const tTag * pTag){
    int             ruleIndex;
    char            scriptName[PATH_MAX];

    /*
     * run through the list of rules: - try to match tags detected tag
     * with regex in rule for tagID IF matches - try to find and execute
     * a script done in reverse order, ending with most generic rules
     */
    for (ruleIndex = (numTagEntries - 1); ruleIndex >= 0; ruleIndex--) {
        if (tagEntryArray[ruleIndex].enabled) {
            ...
            case TAG_IN:
                /*
                 * return TRUE if we were able to execute
                 * script
                 */
