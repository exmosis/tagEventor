...
typedef enum { FOREGROUND, START_DAEMON, STOP_DAEMON} tRunOptions;
...
static void tagDetachData(tTag *pTag){
    
    if ( pTag->contents.pData && pTag->contents.pData != testTag.pData){
        //fprintf(stderr, "pTag->contents.pData = %s\n", pTag->contents.pData);
...
static void tagAttachData(tTag *pTag, tReaderManager *pManager ){
...
void eventDispatch(tEventType eventType, tTag *pTag, int readerNumber, tReaderManager  *pManager){
...
static void processTagLists(void (*callBack)( char, const char *, const char * ) ){
  int			  i, j;
  unsigned char	found;
...
static int tagListCheck(void *data /* pointer to a callback function */ ){
  int 		  rv;
  char			messageString[MAX_LOG_MESSAGE];
